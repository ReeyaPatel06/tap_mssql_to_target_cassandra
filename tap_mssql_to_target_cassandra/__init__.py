...
def process_schema(table_name: str, schema: dict, key_properties: Sequence[str]) -> Model:
    ...
    return type(table_name.replace("-","_"), (Model,), model_attrs)
...
def persist_lines(config, lines):
    ...
    #-----------------------------------------------------------#
    #-----------------------------------------------------------#
    # list of exclude schemas
    EXCLUDE_SCHEMAS = []
    # list of exclude tables
    EXCLUDE_TABLES = []

    # check exclude_schemas in config (meltano.yaml)
    if "exclude_schemas" in list(config.keys()):
        # get exclude_schemas from meltano.yaml
        EXCLUDE_SCHEMAS = config["exclude_schemas"]

    # check exclude_tables in  config (meltano.yaml)
    if "exclude_tables" in list(config.keys()):
        # get exclude_tables list from: meltano.yaml
    ...
    for line in lines:
        ...
        #-----------------------------------------------------------#
        if 'stream' in o:
            # get table name e.g. dbo-student
            stream = o['stream']
            # table name & schema name from mssql
            d_schema = ""
            d_table = ""
            # stream has schema & table name: eg. dbo-student so we split them by -
            if len(stream.split("-"))==2:
                ts = stream.split("-") # split name by -
                d_schema = ts[0] 
                # create table name with ts[0] is schema & ts[1] table name
                d_table = ts[0]+"_"+ts[1]

            # if schema in exclude list loop skip
            if d_schema in EXCLUDE_SCHEMAS: 
                continue
            # if table in exclude list loop skip
            if d_table in EXCLUDE_TABLES:  
                continue
        #-----------------------------------------------------------#
        #-----------------------------------------------------------#
